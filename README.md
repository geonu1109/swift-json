...
> The directory and repo are named `swift-json`. It ships two modules:
>
> ```swift
> import JSON            // the JSON value model + JSONSchema types — no dependencies
> import JSONSchemaMacro // adds the @JSONSchemaModel macro (re-exports JSON)
> ```
...
.target(name: "YourTarget", dependencies: [
    .product(name: "JSON", package: "swift-json"),              // values + JSONSchema types
    // .product(name: "JSONSchemaMacro", package: "swift-json"), // adds @JSONSchemaModel
]),
```

`JSON` has **no external dependencies**. `JSONSchemaMacro` pulls in [swift-syntax](https://github.com/swiftlang/swift-syntax) for the macro, so depend on it only where you use `@JSONSchemaModel` — and `import JSONSchemaMacro` re-exports `JSON`.
...
`import JSONSchemaMacro` (it re-exports `JSON`), then attach `@JSONSchemaModel` to a `struct`, `class`, or raw-value `enum` to synthesize a `JSONSchemaRepresentable` conformance, giving the type a `static var jsonSchema: JSONSchema`.
...
- `$defs` keys use the type's simple name; two distinct types sharing a name get a numeric suffix (`Foo`, `Foo_2`).
- Type inference covers scalar literals (`Int`, `Double`, `String`, `Bool`); other un-annotated properties need an explicit annotation (you'll get a build warning for any that were skipped).

## Requirements

Swift 6.0+ · macOS 13 / iOS 16 / tvOS 16 / watchOS 9 / visionOS 1. The `JSON` module has no dependencies; `JSONSchemaMacro` depends on [swift-syntax](https://github.com/swiftlang/swift-syntax).
