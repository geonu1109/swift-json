...
private extension JSON {
    static func write(_ value: JSON, into output: inout String, prettyPrinted: Bool, indent: Int) {
        ...
        case let .number(number):
            // Non-finite values aren't representable in JSON; emit null rather
            // than the invalid `inf`/`nan` tokens. (The parser never produces
            // these — only programmatic construction can.)
