...
> 디렉터리·repo 이름은 `swift-json`이고, 두 개의 모듈을 제공한다.
>
> ```swift
> import JSON            // JSON 값 모델 + JSONSchema 타입 — 의존성 없음
> import JSONSchemaMacro // @JSONSchemaModel 매크로 추가 (JSON을 재노출)
> ```
...
.target(name: "YourTarget", dependencies: [
    .product(name: "JSON", package: "swift-json"),              // 값 + JSONSchema 타입
    // .product(name: "JSONSchemaMacro", package: "swift-json"), // @JSONSchemaModel 추가
]),
```

`JSON`은 **외부 의존성이 없다.** `JSONSchemaMacro`는 매크로를 위해 [swift-syntax](https://github.com/swiftlang/swift-syntax)를 끌어오므로, `@JSONSchemaModel`을 쓰는 곳에서만 의존한다 — `import JSONSchemaMacro`가 `JSON`을 재노출한다.
...
`import JSONSchemaMacro`(JSON을 재노출) 후, `struct`·`class`·raw-value `enum`에 `@JSONSchemaModel`을 붙이면 `JSONSchemaRepresentable` 준수가 생성되고 타입에 `static var jsonSchema: JSONSchema`가 추가된다.
...
- **매크로**는 별도 모듈(`JSONSchemaMacro`)에 있어 코어 `JSON` 모듈을 의존성 없이 유지한다. [swift-syntax](https://github.com/swiftlang/swift-syntax) 컴파일러 플러그인이며, 각 모델은 진행 중인 타입을 추적하는 `JSONSchemaContext`를 통해 스키마를 기여한다 — 순환은 `$defs`의 `$ref`로 끊고, 비순환 타입은 inline로 둔다.
...
- `$defs` 키는 타입의 단순 이름을 쓴다. 서로 다른 두 타입이 같은 이름을 가지면 두 번째에 숫자 접미사가 붙는다(`Foo`, `Foo_2`).
- 타입 추론은 스칼라 리터럴(`Int`, `Double`, `String`, `Bool`)까지 지원한다. 그 외 어노테이션 없는 프로퍼티엔 명시적 타입 어노테이션을 추가한다(누락분은 빌드 경고로 표시된다).

## 요구 사항

Swift 6.0+ · macOS 13 / iOS 16 / tvOS 16 / watchOS 9 / visionOS 1. `JSON` 모듈은 의존성이 없고, `JSONSchemaMacro`는 [swift-syntax](https://github.com/swiftlang/swift-syntax)에 의존한다.
